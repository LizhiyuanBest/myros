...
sensor_msgs::Image::Ptr depth_img;
sensor_msgs::Image::Ptr color_img;
sensor_msgs::PointCloud2 point_cloud;

// service 回调函数，输入参数req，输出参数res
bool cloudCallback(mykinect2::mykinect::Request &req, mykinect2::mykinect::Response &res)
{
	//显示请求数据
	ROS_INFO("kinect[name:%s ]", req.name.data.c_str());

	//设置反馈数据
	res.result = point_cloud;
...
int main(int argc, char * argv[]){
  ...
  //创建一个名为/kinect2/cloud的server, 注册回调函数 cloudCallback
	ros::ServiceServer cloud_service = n.advertiseService("/kinect2/cloud", cloudCallback);
  ROS_INFO("kinect server ");
	//设置循环刷新的频率
