...
void myprocess(pcl::PointCloud<PointT>::Ptr cloud_src)
{
	...
	const std::string src_file[3] = {"/home/li/ROS/myros/cloud/cloud_cluster_3.pcd","/home/li/ROS/myros/cloud/cloud_cluster_0.pcd", "/home/li/ROS/myros/cloud/cloud_cluster_5.pcd"};
	...
	{ //迭代容器中的点云的索引，并且分开保存索引的点云
		...
		if((float)cloud_tgt->points[index].r>2*(float)cloud_tgt->points[index].g && (float)cloud_tgt->points[index].r>2*(float)cloud_tgt->points[index].b)
		{//red
			pcl::io::loadPCDFile(src_file[0], *cloud_src);	//加载点云文件
		}
		else if ((float)cloud_tgt->points[index].r>2*(float)cloud_tgt->points[index].b && (float)cloud_tgt->points[index].g>2*(float)cloud_tgt->points[index].b)
        {//yellow
			pcl::io::loadPCDFile(src_file[0], *cloud_src);	//加载点云文件
		}
		else if (((float)cloud_tgt->points[index].r-(float)cloud_tgt->points[index].g)<30.0 && ((float)cloud_tgt->points[index].r-(float)cloud_tgt->points[index].b)<30.0)
		{//white
        ...
        pose_msg.data.push_back((float)j);
        pose_msg.data.push_back(model);
        pose_msg.data.push_back(0.0);
        pose_msg.data.push_back(0.0);
        //pose_msg = temp_msg;
...
void kinect2CloudCallback(const sensor_msgs::PointCloud2::ConstPtr& msg)
{
    ...
    if (process_status == NEXT){
        try{
            ...
            myprocess(cloud);
            std::cout<<"try callback process_status:"<< process_status<<endl;
        }
        catch (std::exception &e) {
            pose_msg.data.clear();
            position_msg.data.clear();
            process_status = NEXT;
            std::cout<<"catch callback process_status:"<< process_status<<endl;
        }
    }
    ...
    //     pcl::io::savePLYFileASCII("/home/li/ROS/myros/cloud/pointcloud.ply", *cloud);
...
int main(int argc, char * argv[]){
	...
	ros::Subscriber kinect2_cloud_sub = n.subscribe("/kinect2/hd/points",1,kinect2CloudCallback);


    //创建一个publisher，发布名为/process/pose的topic，
    //消息类型为 std_msgs::Float32MultiArray，队列长度为1
    ros::Publisher pose_pub = n.advertise<std_msgs::Float64MultiArray>("/process/pose",1);
    ros::Publisher position_pub = n.advertise<std_msgs::Float64MultiArray>("/process/position",1);
    ...
    while(ros::ok()){
        ...
        // pose_msg.data.push_back(1.0);
        // pose_msg.data.push_back(5.0);
