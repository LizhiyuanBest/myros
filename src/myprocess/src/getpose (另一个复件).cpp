...
pcl::PointCloud<PointT>::Ptr cloud_src_to_show(new pcl::PointCloud<PointT>);//显示用的模板点云
pcl::PointCloud<PointT>::Ptr cloud_res_to_show(new pcl::PointCloud<PointT>);//显示用的结果点云
pcl::PointCloud<PointT>::Ptr cloud_res_to_pub(new pcl::PointCloud<PointT>);//显示用的结果点云
...
//图像处理函数
void myprocess(pcl::PointCloud<PointT>::Ptr cloud_src, ros::Publisher point_cloud_pub)
{
...
//        cout << "error in aixs_x: " << error_x << "  error in aixs_y: " << error_y << "  error in aixs_z: " << error_z << endl;
        ...
        *cloud_res_to_pub += *cloud_tgt_to_show;
        ...
        pcl::toROSMsg(*cloud_res_to_pub, point_cloud_2);
...
int main(int argc, char * argv[]){
    ...
    ros::Publisher point_cloud_pub = n.advertise<sensor_msgs::PointCloud2>("/process/points", 1);
    ...
    while(ros::ok()){
        ...
        if (process_status == NEXT){
                process_status = START;
                myprocess(cloud,point_cloud_pub);
        }
